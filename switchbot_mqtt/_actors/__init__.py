...
class _ButtonAutomator(_MQTTControlledActor):
    ...
    async def _connect(self) -> None:
        device = await BleakScanner.find_device_by_address(self._mac_address)

        self.__device = switchbot.Switchbot(
            device=device, password=self._password, retry_count=self._retry_count
        )
        ...
        if mqtt_message_payload.lower() == b"on":
            if not self.__device.turn_on():
                ...
                if update_device_info:
                   await self._update_and_report_device_info(mqtt_client, mqtt_topic_prefix)
...
class _CurtainMotor(_MQTTControlledActor):
    ...
    async def _connect(self) -> None:
        ...
        self.__device = switchbot.SwitchbotCurtain(
            device=device, password=self._password, retry_count=self._retry_count, reverse_mode=True
        )
    ...
    async def _report_position(
        self,
        mqtt_client: asyncio_mqtt.Client,  # pylint: disable=duplicate-code; similar param list
        mqtt_topic_prefix: str,
    ) -> None:
